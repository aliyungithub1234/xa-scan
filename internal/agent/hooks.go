...
func RegisterDefaultHooks(reg *HookRegistry) {
	// Order matters: security → tracking → detection → policy → reset
	reg.Register(OnToolCall, hookSecurityGuard)
	...
	reg.Register(OnToolResult, hookSecurityVulnFilter)
	...
	reg.Register(OnIterationStart, hookSecurityPolicyReminder)
...
// ── hookSecurityGuard ────────────────────────────────────────────────────────
// Enforces the 11-rule ethical pentesting policy at the agent decision layer.
// Fires on OnToolCall before command execution. Blocks dangerous tool choices
// that bypass the terminal command filter (e.g. through encoded parameters).
func hookSecurityGuard(state *ScanState, args map[string]string) HookResult {
	toolName := args["tool_name"]

	// Only inspect terminal commands — other tools have their own safety
	if toolName != "terminal_execute" {
		return HookResult{}
	}
	...
	// Layer 2 check: apply security guard patterns
	guard := security.CheckCommand(cmd)
	if guard.Blocked {
		return HookResult{
			Block:       true,
			BlockReason: guard.Reason,
			EmitMessage: fmt.Sprintf("🛡️ SECURITY GUARD: %s", guard.Reason),
		}
	}

	// Check SQL injection payloads for UPDATE/DELETE/INSERT/DROP
	lower := strings.ToLower(cmd)
	if strings.Contains(lower, "sqlmap") || strings.Contains(lower, "' or ") ||
		strings.Contains(lower, "' and ") || strings.Contains(lower, "union") {
		sqlGuard := security.CheckSQLInjection(cmd)
		if sqlGuard.Blocked {
			return HookResult{
				Block:       true,
				BlockReason: sqlGuard.Reason,
				EmitMessage: fmt.Sprintf("🛡️ SQL GUARD: %s", sqlGuard.Reason),
			}
		}
	}
...
// ── hookSecurityVulnFilter ───────────────────────────────────────────────────
// Filters low-value vulnerabilities that should not be reported (Rule #12).
// Fires on OnToolResult after vulnerability discovery.
func hookSecurityVulnFilter(state *ScanState, args map[string]string) HookResult {
	output := args["output"]
	if output == "" {
		return HookResult{}
	}

	// Check if the output describes a low-value vulnerability
	if ignored, reason := security.IsIgnoredVuln(output); ignored {
		return HookResult{
			Nudge: fmt.Sprintf("🛡️ VULNERABILITY FILTER: This finding is classified as low-value and should NOT be reported: %s. Focus on high-impact vulnerabilities (SQLi, RCE, Auth Bypass, IDOR, SSRF, etc.).", reason),
		}
	}
...
// ── hookSecurityPolicyReminder ───────────────────────────────────────────────
// Reminds the agent of the ethical policy at regular intervals.
// Fires on OnIterationStart at iteration 1, 10, and 30.
func hookSecurityPolicyReminder(state *ScanState, args map[string]string) HookResult {
	iter := state.Iteration

	// Remind at key milestones
	if iter == 1 || iter == 10 || iter == 30 {
		nudge := `🛡️ ETHICAL POLICY REMINDER:
You are a responsible security tester. Your mission: PROVE vulnerabilities exist WITHOUT exploiting them.

KEY RULES:
1. READ-ONLY testing — never UPDATE/DELETE/INSERT/DROP data
2. ≤5 records max for sensitive data proof
3. NO DoS, NO phishing, NO privesc, NO lateral movement
4. NO malware upload — text proof files only (1.txt, 1.php)
5. Use your OWN test accounts (max 2) for verification
6. IGNORE low-value findings: URL redirect, self-XSS, weak password, email bombing

If you find RCE/shell access: upload a harmless text file (e.g., echo "poc" > 1.txt) as proof. NEVER upload binaries, reverse shells, or malware.`

		return HookResult{Nudge: nudge}
	}
