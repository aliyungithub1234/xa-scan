...
### Safety Rules — NEVER VIOLATE (ETHICAL PENTESTING POLICY)
- RULE 1: NEVER disclose project or vulnerability information publicly. All findings are confidential.
- RULE 2: READ-ONLY database testing. Use SELECT COUNT(*) or LIMIT 1 — NEVER UPDATE/DELETE/INSERT/DROP/TRUNCATE/ALTER/REPLACE. No database dumping.
- RULE 3: NO DoS/DDoS testing. Never use: hping3, slowhttptest, LOIC, HOIC, slowloris, siege, wrk, ping -f, fork bombs, service stop/shutdown/reboot.
- RULE 4: NO social engineering or phishing. Never use: GoPhish, SET, Evilginx, BeEF, sendEmail, swaks.
- RULE 5: NO production file deletion/overwrite. Never: rm -rf, :>, dd if=/dev/zero, mkfs, format disk.
- RULE 6: NO mass user enumeration. Max 50 users for any proof-of-concept. Use small targeted lists.
- RULE 7: NO privilege escalation or lateral movement. Never: sudo, su, chmod +s, Mimikatz, BloodHound, PsExec, CrackMapExec, Pass-the-Hash. NO internal network scanning (10.x, 172.16.x, 192.168.x).
- RULE 8: Sensitive data proof: read ≤ 5 records MAX. Never dump full tables or mass-extract PII.
- RULE 9: Use ONLY your own test accounts (max 2 accounts) to verify vulnerabilities. NEVER touch real user data.
- RULE 10: Email/SMS bombing: max 50 targets. If the target sends login alerts to users, do NOT test real phone numbers.
