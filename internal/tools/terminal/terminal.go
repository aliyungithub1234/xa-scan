...
func executeCommandWithContextID(contextID string, args map[string]string) (tools.Result, error) {
	...
	// Enforce ethical pentesting policy (11 rules + vulnerability filtering)
	if guard := security.CheckCommand(command); guard.Blocked {
		log.Printf("[SECURITY] Command blocked by guard: %s", guard.Reason)
		return tools.Result{Output: fmt.Sprintf("%s", guard.Reason)}, nil
	}
