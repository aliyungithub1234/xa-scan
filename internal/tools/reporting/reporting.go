...
If you cannot exploit it, downgrade severity to 'info' and report as informational.`,
	...
	// ── Gate 4: Security Policy — filter low-value/ignored vulnerability types (Rule #12) ──
	if ignored, reason := security.IsIgnoredVuln(title); ignored {
		return tools.Result{
			Output: fmt.Sprintf("🛡️ SECURITY FILTER: Vulnerability report rejected (Rule #12).\nTitle: %s\nReason: %s\n\nThis vulnerability type is classified as low-value. Focus on high-impact findings (SQLi, RCE, Auth Bypass, IDOR, SSRF, XXE, etc.).", title, reason),
		}, nil
	}
	if ignored, reason := security.IsIgnoredVuln(args["description"]); ignored {
		return tools.Result{
			Output: fmt.Sprintf("🛡️ SECURITY FILTER: Vulnerability report rejected (Rule #12).\nDescription matches low-value pattern.\nReason: %s\n\nFocus on high-impact vulnerabilities with concrete exploitation proof.", reason),
		}, nil
	}

	// ── Gate 5: Smart Deduplication — same vuln type on same endpoint = duplicate ──
	...
	// ── Gate 6: Severity classification — enforce max severity per vuln type ──
	originalSeverity := ""
	...
	// ── Gate 7: CVSS-to-Severity enforcement (HackerOne standard) ──
	...
	go func() {
		dingVuln := notify.VulnInfo{
			...
			Impact:             vuln.Impact,
			ExploitationProof:  notify.SanitizeProofForNotification(vuln.ExploitationProof),
			VerificationMethod: vuln.VerificationMethod,
			CVSS:               vuln.CVSS,
			CVE:                vuln.CVE,
			Timestamp:          vuln.Timestamp,
			AgentName:          vuln.AgentName,
			Remediation:        vuln.Remediation,
		}
		if err := notify.SendVulnReport(dingVuln); err != nil {
			// Log but don't fail the report
			// log.Printf("[DINGTALK] Failed to send notification: %v", err)
		}
	}()
