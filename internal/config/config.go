...
func (c *Config) Validate() error {
	if c.LLM == "" {
		return fmt.Errorf("XALGORIX_LLM is required. Set it to a model like 'openai/gpt-5.4', 'anthropic/claude-sonnet-4-20250514', 'kimi/kimi-for-coding', or 'moonshot/kimi-for-coding'")
	}
	if c.APIKey == "" {
...
func CheckEnvFile() error {
	...
	if _, err := os.Stat(envPath); os.IsNotExist(err) {
		return fmt.Errorf("configuration file not found: %s\n\nPlease create it with:\n  XALGORIX_LLM=minimax/MiniMax-M2.7\n  XALGORIX_API_KEY=your_api_key\n\nOr run: xalgorix --setup\n\nSupported providers include: openai, anthropic, deepseek, groq, google, gemini, minimax, ollama, kimi, moonshot", envPath)
	}
	...
	if llm == "" || apiKey == "" {
		return fmt.Errorf("configuration file is invalid or missing required variables\n\nPlease add to %s:\n  XALGORIX_LLM=minimax/MiniMax-M2.7\n  XALGORIX_API_KEY=your_api_key\n\nSupported: openai/gpt-5.4, anthropic/claude-sonnet-4-20250514, kimi/kimi-for-coding, moonshot/kimi-for-coding, minimax/MiniMax-M2.7, deepseek/deepseek-chat", envPath)
	}
